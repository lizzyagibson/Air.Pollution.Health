...
all_long %>% filter(!(parameter_name %in% 
                        c("Humidity", "Temp", "NO2", "PM2.5", "SO2", "Ozone", "CO"))) %>%
  pull(arithmetic_mean) %>% summary()
...
autoplot(scale(pr.out$rotation)) + 
  labs(y = "Pollutants", x = "Principle Components", 
                                        fill = "Value", 
                                        title = "Individual pollutant loadings on principle components") +
...
#   labs(title = "Biplot of first two principle components")
#biplot of first 2 principle components
...
melted <- melt(pr.out$rotation[,1:23]) %>% 
  mutate(variable.group = ifelse(Var1 %in% c("CO", "SO2", "NO2", "Ozone"), "Criteria Gases",
                                 ifelse(Var1 == "PM2.5", "Particulates", "PM Species")))
...
melted %>% filter(Var2 %in% c("PC1", "PC2", "PC3", "PC4")) %>% ggplot() +
  ...
  facet_wrap(~Var2) + theme(axis.text.x = element_text(angle = 90, hjust = 1), 
    ...
    labs(y = "Value", x = "Pollutants", title = "Variable Loadings on Principle Components",
         fill = "Group")
```
