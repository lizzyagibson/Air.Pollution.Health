...
All of the predictor variables from the cleaned dataset were used except for three variables were used in the supervised analyses (total PM 2.5 mass concentrations, ozone, sulfate, nitrate, aluminum, calcium, iron, silicon, lead, manganese, zinc, bromine, copper, nickel, sulfur, vanadium and temperature). The three variables excluded were day of the week, humidity and date. For our supervised analyses we used boosted random forests and lasso models. The dataset was split into a training and test sets in order to perform cross-validation for tuning parameters, fitting the models, and testing their performances.
...
We use the train dataset to fit the lasso model and find the best lambda value using cross-validation. Then use the test dataset to test the performance of our model. 

**Results** 

The best lambda value that we found through cross-validation is 4.479. In Figure 11, we can also see that when testing multiple data values in the training dataset, the lambda value that produces the lowest mean square error (MSE) is around 4. The lasso model with the lambda chosen by cross-validation contains only 10 variables which are presented in Table 3. 
